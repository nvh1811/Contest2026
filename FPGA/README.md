...
**Sơ đồ luồng dữ liệu (Data Path):**
```text
 S1 (WDI) ──► debounce_button ──┐
                                ├──► watchdog_core ──► LED (WDO)
 S2 (EN)  ──► debounce_button ──┤                  └──► LED (ENOUT)
                                │
 UART RXD ──► uart_rx ──► frame_parser (tích hợp Regs) ──┐
                                │                        │
 UART TXD ◄── uart_tx ◄── uart_frame_responder ◄─────────┘

```
**Sơ đồ truyền nhận qua UART**

![Sơ đồ Data Flow UART](Documents/DATA_FLOW_UART.drawio.png)
...
### 3.4. Thông số khởi tạo mặc định (Default Parameters)
Ngay khi cấp nguồn hoặc Reset, hệ thống hoạt động với các thông số an toàn sau (có thể ghi đè qua UART):

| Thông số | Giá trị mặc định | Mô tả |
| :--- | :--- | :--- |
| **tWD_ms** | `1600` | Thời gian Timeout tối đa của Watchdog (ms) |
| **tRST_ms** | `200` | Thời gian kéo giữ tín hiệu lỗi WDO (ms) |
| **arm_delay_us** | `150` | Thời gian chờ "làm ngơ" WDI sau khi Enable (us) |
---
...
### 6.1. FSM Lõi Watchdog (Watchdog Core)
![Sơ đồ FSM Watchdog](Documents/FSM_Watchdog.png)

### 6.2. FSM Khối nhận dữ liệu UART (UART RX)
![Sơ đồ FSM UART RX](Documents/FSM_UART_RX.drawio.png)

### 6.3. FSM Bộ phân giải gói tin (Frame Parser)
![Sơ đồ FSM Frame Parser](Documents/UART_FRAME_PARSER.drawio.png)

### 6.4. FSM Bộ đóng gói phản hồi (Frame Responder)
![Sơ đồ FSM Frame Responder](Documents/UART_FRAME_RESPONDER.drawio.png)
---
...
## 10. Cấu trúc Thư mục (File Structure)

```text
📦 FPGA
 ┣ 📂 Documents             # Chứa tài liệu, hình ảnh minh hoạ cho file README
 ┣ 📂 impl                  # Thư mục chứa các file tổng hợp (Synthesis) và bitstream 
 ┣ 📂 sim                   # Thư mục phụ trợ cho quá trình mô phỏng (nếu có)
 ┣ 📂 src                   # Chứa mã nguồn RTL, Testbench và các file cấu hình
 ┃ ┣ 📜 ContestFPGA.cst     # File ràng buộc chân vật lý (Physical Constraints)
 ┃ ┣ 📜 ContestFPGA.rao     # File cấu hình Gowin Analyzer Oscilloscope (Debug)
 ┃ ┣ 📜 ContestFPGA.sdc     # File ràng buộc timing (Timing Constraints)
 ┃ ┣ 📜 sync_debounce.v     # Khối đồng bộ và lọc nhiễu nút nhấn
 ┃ ┣ 📜 tb_disable.v        # File testbench kiểm tra trạng thái vô hiệu hoá
 ┃ ┣ 📜 tb_disable_to_enable.v # File testbench kiểm tra chuyển đổi trạng thái
 ┃ ┣ 📜 tb_normal_kick.v    # File testbench kiểm tra kick định kỳ
 ┃ ┣ 📜 tb_timeout.v        # File testbench mô phỏng lỗi treo máy
 ┃ ┣ 📜 tb_uart.v           # File testbench giao thức UART toàn hệ thống
 ┃ ┣ 📜 uart_engine.v       # Module thu/phát tín hiệu UART (RX/TX)
 ┃ ┣ 📜 uart_frame_parser.v # Bộ phân giải Frame & Thanh ghi
 ┃ ┣ 📜 uart_frame_responder.v # Bộ đóng gói dữ liệu phản hồi
 ┃ ┣ 📜 watchdog_core.v     # Lõi xử lý FSM Watchdog
 ┃ ┗ 📜 watchdog_main.v     # Module cấp cao nhất (Top-level)
 ┣ 📜 ContestFPGA.gprj      # File Project chính của Gowin FPGA Designer
 ┣ 📜 ContestFPGA.gprj.user # File lưu cấu hình giao diện người dùng
