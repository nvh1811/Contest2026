...
int8_t hh = 0, mm = 0, ss = 0; // Thoi gian thuc 
int8_t al_h = 0, al_m = 0;     // Thoi gian bao thuc
...
void Buzzer_Task(void) {
    // Bao thuc keu 5s/chu ky (0.5s ON - 0.5s OFF)
    if (alarm_ring_ms > 0) {
        alarm_ring_ms--;
        if ((alarm_ring_ms % 1000) >= 500) SN_CT16B0->MR0 = 1500; // ON
        else SN_CT16B0->MR0 = 0;                                  // OFF
        return; 
    }
    // Bip 0.3s (300ms) khi an nut hoac timeout
    if (beep_timer_ms > 0) {
        beep_timer_ms--;
        SN_CT16B0->MR0 = 1500; 
    } else {
...
int main(void) {
    ...
    // SETUP CHO LED D6 (P3.8 - Active Low) 
    SN_GPIO3->MODE |= (1 << 8); 
    ...
    while(1) {
        if(timer_1ms_flag) {
            ...
            if (ms_count >= 1000) {
                ms_count = 0;
                ss++;
                if (ss >= 60) { ss = 0; mm++; } // Dem phut 
                if (mm >= 60) { mm = 0; hh++; } // Dem gio 
                if (hh >= 24) hh = 0; 
            ...
            // TIMEOUT 30s THOAT CAI DAT 
            if (current_mode != MODE_NORMAL) {
                timeout_ms++;
                if (timeout_ms >= 30000) {
                    current_mode = MODE_NORMAL;
                    beep_timer_ms = 300; // Keu 0.3s khi thoat 
                    timeout_ms = 0;
                }
            }
            ...
            if (key_code) {
                if (key_code == KEY_SETUP || key_code == KEY_ALARM || key_code == KEY_UP || key_code == KEY_DOWN) {
                    timeout_ms = 0;      
                    alarm_ring_ms = 0;   
                    beep_timer_ms = 300; // KEU 0.3s MOI KHI AN NUT 
                    
                    switch(key_code) {
                        case KEY_SETUP:
                            if (current_mode == MODE_NORMAL) current_mode = MODE_SET_H; 
                            else if (current_mode == MODE_SET_H) current_mode = MODE_SET_M; 
                            else current_mode = MODE_NORMAL; 
                            break;
                            
                        case KEY_ALARM:
                            if (current_mode == MODE_NORMAL) current_mode = MODE_AL_H; 
                            else if (current_mode == MODE_AL_H) current_mode = MODE_AL_M; 
                            else { 
                                current_mode = MODE_NORMAL; 
                                EEPROM_Save_Alarm(); // Luu thong so 
                            }
                            break;
                            
                        case KEY_UP:
                            if (current_mode == MODE_SET_H) { hh++; if(hh>=24) hh=0; } 
                            else if (current_mode == MODE_SET_M) { mm++; if(mm>=60) mm=0; } 
                            else if (current_mode == MODE_AL_H)  { al_h++; if(al_h>=24) al_h=0; }
                            else if (current_mode == MODE_AL_M)  { al_m++; if(al_m>=60) al_m=0; }
                            break;
                            
                        case KEY_DOWN:
                            if (current_mode == MODE_SET_H) { hh--; if(hh<0) hh=23; } 
                            else if (current_mode == MODE_SET_M) { mm--; if(mm<0) mm=59; } 
