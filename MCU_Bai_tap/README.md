...
## 3. Kiến trúc Phần mềm & Máy trạng thái

#### 3.1. Cơ chế điều phối tác vụ (Task Scheduling)
Hệ thống vận hành dựa trên mô hình **Super-loop** kết hợp với **Timer Interrupt** để quản lý đa tác vụ thời gian thực mà không gây nghẽn luồng xử lý:

- **Interrupt Service Routine (ISR):** Sử dụng Timer **CT16B1** tạo ngắt mỗi **1ms**. Trình phục vụ ngắt sẽ bật cờ `timer_1ms_flag`.
- **Main Loop:** CPU liên tục kiểm tra cờ này để thực hiện các tác vụ định kỳ: Quét LED, quét phím, cập nhật thời gian và xử lý âm thanh.

**Sơ đồ luồng điều phối tác vụ:**

<img width="2204" height="3484" alt="image" src="https://github.com/user-attachments/assets/708f9546-4daa-432f-bdbd-fe25237512d4" />


#### 3.2. Máy trạng thái hữu hạn (Finite State Machine - FSM)
